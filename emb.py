...
class EmbeddingWithPosition(nn.Module):
    def __init__(self,vocab_size,emb_size,dropout=0.1,seq_max_len=5000):
        ...
        # 为序列中每个位置准备一个位置向量，也是emb_size宽    (seq_max_len,emb_size)
        position_idx=torch.arange(0,seq_max_len,dtype=torch.float).unsqueeze(-1) #（5000，） -> (5000,1)
        position_emb_fill=position_idx*torch.exp(-torch.arange(0,emb_size,2)*math.log(10000.0)/emb_size)   #position_emb_fill:(seq_max_len, emb_size/2)
    ...
    def forward(self,x):    # x: (batch_size,seq_len)
        x=self.seq_emb(x)   # x: (batch_size,seq_len,emb_size)   每个词向量的宽度为emb_size，共有seq_len个
        #pos_encoding=(1,seq_max_len,emb_size)  x.size()[1]=seq_len
